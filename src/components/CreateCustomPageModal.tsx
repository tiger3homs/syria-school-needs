...
const CreateCustomPageModal = ({ open, onOpenChange, onPageCreated }: CreateCustomPageModalProps) => {
  ...
  const generateSlugFromSubject = (subject: string) => {
    return subject
      .toLowerCase()
  ...
  const handleSubjectChange = (value: string) => {
    ...
    setFormData(prev => ({
      ...prev,
      subject: sanitized,
      slug: prev.slug || generateSlugFromSubject(sanitized)
    }));
    if (errors.subject) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description: t('admin.customPages.createSuccess'),
      });
      ...
      toast({
        ...
        description: t('admin.customPages.createError'),
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{t('admin.customPages.createNew')}</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className={`flex gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
            <Button type="submit" disabled={loading}>
              {loading ? t('common.saving') : t('admin.customPages.create')}
            </Button>
            <Button
              type="button"
              variant="outline"
